...
\begin{titlepage}
    \centering
    \vspace*{1.25in}
    {\Large\textbf{Robustness Audit of Health Misinformation Classification Models: Progress Report}\par}
    \vspace{0.75in}
    {Mustafa Yousuf\par}
    \vspace{0.25in}
...
\begin{abstract}
\noindent Health misinformation classifiers are often evaluated through aggregate within-dataset accuracy, but such results may reflect dataset artifacts rather than robust health-information signals. This progress report describes the first half of a study evaluating whether traditional machine-learning and transformer-based models can classify dataset-defined unreliable health information under standard and artifact-controlled conditions. CoAID is used as the primary COVID-19 health-information dataset, and FakeHealth is used as a secondary broader health-news dataset. Completed work includes data acquisition, provenance auditing, label harmonization, processed-table construction, leakage-aware split generation, engagement-feasibility assessment, and traditional text baselines. The current results show strong CoAID baseline performance and lower FakeHealth performance, indicating that task difficulty and label provenance differ substantially across datasets. Transformer evaluation is implemented but remains pending because the local environment lacks the required transformer dependencies.
\end{abstract}
...
The narrative and explainability literature adds two cautions. Chen et al. (2022) show that COVID-19 misinformation can be organized into recurring narratives at large scale, while Langguth et al. (2023) show that topic labels are not enough because content can mention a conspiracy without endorsing it. Athira et al. (2023) further argues that explainable artificial intelligence methods should be evaluated with attention to the intended user and decision context. In the present study, explanations are treated as audit evidence rather than causal proof of model reasoning.

\section*{Hypotheses}

The primary hypothesis is that traditional text models will produce strong within-dataset performance, especially on CoAID, but that this performance will not be sufficient evidence of reliable health-misinformation understanding. Some performance is expected to persist under artifact-masked or controlled evaluation, but the results are also expected to show dataset dependence. CoAID models may benefit from COVID-specific vocabulary and source structure, while FakeHealth models may face a harder and broader task because its labels arise from health-news review ratings rather than a single pandemic-era topic.

A secondary hypothesis is that social engagement features will be useful only conditionally. Engagement behavior may contain signal, but metadata coverage and missingness can themselves become artifacts. Therefore, engagement should be used only after a feasibility gate checks coverage, class balance, and proxy risk. A third hypothesis is that transformer models may improve text classification, but only if they are evaluated against the same frozen splits as the traditional baselines.
